...
import { deleteOpeningHoursBlock } from "./actions";
import type { ActionResult } from "@/components/action-form";

export function DeleteBlockButton({ id, venueId }: { id: string; venueId: string }) {
  const [, formAction, pending] = useActionState<ActionResult, FormData>(
    async (_prevState, formData) => deleteOpeningHoursBlock(formData),
    undefined,
  );
