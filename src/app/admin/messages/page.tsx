...
/**
 * Cross-venue version of the per-venue Messages tab
 * (/staff/[venueSlug]/messages): every booking with at least one unread
 * inbound message, across all venues, for the admin top nav. OWNER/MANAGER
 * only (requireAdminSession, and the admin layout, both block STAFF), so
 * it's deliberately unscoped, that's exactly the "an admin sees unread
 * messages for all venues" half of Andy's spec; a venue-scoped STAFF
 * account uses the per-venue page instead, which shows only their venue.
 */
export default async function AdminMessagesPage() {
  await requireAdminSession();

  const bookings = await prisma.booking.findMany({
    where: { messages: { some: { direction: "INBOUND", read: false } } },
    include: {
  ...
  return (
    <div className="animate-in mx-auto max-w-4xl px-4 py-8 sm:px-8 sm:py-10">
      ...
      <p className="mt-1 text-sm text-zinc-500">Bookings with an unread customer reply, across every venue.</p>
      ...
      {bookings.length === 0 ? (
        ...
        <div className="mt-6 flex flex-col gap-2">
          {bookings.map((booking) => {
            ...
            return (
              <Link key={booking.id} href={`/staff/${booking.venue.slug}/bookings/${booking.id}`}>
                <Card interactive className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-zinc-500">
                      {booking.venue.name} · {booking.bookingType.name} · {formatDate(booking.date)}, {booking.startTime}
                    </p>
