...
export default async function StaffDashboardPage() {
  // Belt-and-braces: middleware.ts already blocks an unauthenticated
  // request from reaching this page, but a page that reads the identity
  // directly off the cookie shouldn't just assume middleware ran.
  const session = await getCurrentStaffSession();
  if (!session) redirect("/login");

  const bookings = await getUpcomingBookingsForVenue(session.venueId);
    ...
    <div className="flex flex-1 flex-col bg-zinc-50 px-4 py-8 sm:py-12">
      <div className="mx-auto w-full max-w-3xl">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-zinc-900">{session.venueName}</h1>
