...
/**
 * The shared content of the booking-details view, rendered either as the
 * standalone page (mode "page", see ./page.tsx) or inside the diary's
 * intercepted overlay (mode "modal", see
 * @modal/(.)bookings/[id]/page.tsx) - one component so the two never
 * visually drift apart, per the parity review's section 14/15 finding
 * that having two different layouts for the same booking is exactly what
 * made the system feel unfamiliar to staff.
 */
export async function BookingDetailsBody({
  venueSlug,
  id,
  mode = "page",
}: {
  venueSlug: string;
  id: string;
  /** "page": the original full-page back link. "modal": a close button instead, used by the intercepted booking overlay - see @modal/(.)bookings/[id]/page.tsx. */
  mode?: "page" | "modal";
}) {
  ...
  return (
    <>
        {mode === "page" ? (
          ...
          <CloseModalButton venueSlug={venueSlug} dateStr={booking.date.toISOString().slice(0, 10)} />
        )}
